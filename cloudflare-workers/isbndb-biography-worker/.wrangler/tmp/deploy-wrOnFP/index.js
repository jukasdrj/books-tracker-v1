...
var ISBNdbWorker = class {
  static {
    __name(this, "ISBNdbWorker");
  }
  constructor(env) {
    this.env = env;
  }
  ...
  async getAuthorBibliography(authorName) {
    try {
      console.log(`\u{1F527} RPC: getAuthorBibliography("${authorName}")`);
  ...
  async getBookDetails(isbn) {
    try {
      console.log(`\u{1F527} RPC: getBookDetails("${isbn}")`);
  ...
  async searchBooksByTitle(title) {
    try {
      console.log(`\u{1F527} RPC: searchBooksByTitle("${title}")`);
...
async function handleAuthorRequest(request, env, path, url) {
  ...
  try {
    ...
    const processedBooks = processedData.works.flatMap(
      (work) => work.editions.map((edition) => ({
        ...edition,
...
__name(handleAuthorRequest, "handleAuthorRequest");
async function handleBookRequest(request, env, path, url) {
  ...
  try {
    ...
    await env.KV_CACHE.put(cacheKey, JSON.stringify({ ...bookData, timestamp: (/* @__PURE__ */ new Date()).toISOString() }), {
      expirationTtl: 86400
    });
...
__name(handleBookRequest, "handleBookRequest");
async function handleBooksRequest(request, env, path, url) {
  ...
  try {
    ...
    await env.KV_CACHE.put(cacheKey, JSON.stringify({ ...booksData, timestamp: (/* @__PURE__ */ new Date()).toISOString() }), {
      expirationTtl: 86400
    });
...
__name(handleBooksRequest, "handleBooksRequest");
async function handleSearchRequest(request, env, path, url) {
  ...
  try {
    ...
    await env.KV_CACHE.put(cacheKey, JSON.stringify({ ...searchData, timestamp: (/* @__PURE__ */ new Date()).toISOString() }), {
      expirationTtl: 86400
    });
...
__name(handleCacheRequest, "handleCacheRequest");
...
__name(fetchAuthorBibliography, "fetchAuthorBibliography");
...
__name(fetchBookByISBN, "fetchBookByISBN");
...
__name(fetchBooksByTitle, "fetchBooksByTitle");
...
__name(fetchCombinedSearch, "fetchCombinedSearch");
function calculateEditionScore(book, originalISBN) {
  ...
  if (!publisher.includes("createspace") && !publisher.includes("publishamerica") && !publisher.includes("lightning source") && !publisher.includes("independently published")) {
    score += 20;
  }
  const title = (book.title || "").toLowerCase();
  if (!title.includes("study guide") && !title.includes("classroom") && !title.includes("large print") && !title.includes("companion") && !title.includes("workbook") && !title.includes("test prep") && !title.includes("exam guide")) {
    score += 15;
  }
...
__name(calculateEditionScore, "calculateEditionScore");
function normalizeWorksFromISBNdb(books, searchAuthor) {
  if (!books || !Array.isArray(books)) {
    return { works: [], authors: [] };
  }
  const worksMap = /* @__PURE__ */ new Map();
  ...
  books.forEach((book) => {
    ...
    authorsList.forEach((authorName) => {
      ...
      if (!authorsMap.has(authorKey)) {
        authorsMap.set(authorKey, {
          ...
          identifiers: {
            openLibraryID: null,
            // To be filled by OpenLibrary integration
            isbndbID: null,
            // ISBNdb doesn't provide author IDs consistently
            googleBooksID: null,
            // To be filled by Google Books integration
            goodreadsID: null
            // Future integration
          },
    ...
    if (!worksMap.has(workKey)) {
      worksMap.set(workKey, {
        ...
        identifiers: {
          openLibraryID: null,
          // To be filled by OpenLibrary integration
          isbndbID: book.id || book.isbn13,
          // Use book ID or ISBN as work identifier
    ...
    const existingEdition = work.editions.find(
      (e) => e.isbn === edition.isbn || e.isbns.some((isbn) => edition.isbns.includes(isbn))
    );
    if (!existingEdition) {
  ...
  authors.forEach((author) => {
    author.works = works.filter((work) => work.authors.some(
      (workAuthor) => workAuthor.name.toLowerCase() === author.name.toLowerCase()
    )).map((work) => ({
...
__name(normalizeWorksFromISBNdb, "normalizeWorksFromISBNdb");
function generateWorkKey(title, primaryAuthor) {
  ...
  const cleanAuthor = primaryAuthor.toLowerCase().replace(/[^a-z0-9\s]/g, "").replace(/\s+/g, "-").substring(0, 30);
  return `${cleanTitle}-${cleanAuthor}`;
}
__name(generateWorkKey, "generateWorkKey");
...
__name(collectISBNs, "collectISBNs");
...
__name(extractEditionTitle, "extractEditionTitle");
