...
public struct ContentView: View {
    ...
    public var body: some View {
        ...
        .overlay(alignment: .bottom) {
            if isEnriching {
                // FIX: Use ZStack to prevent overlay from blocking touches
                ZStack {
                    // 1. Transparent layer allowing touches to pass through to underlying TabView
                    Color.clear
                        .contentShape(Rectangle())
                        .allowsHitTesting(false) // CRITICAL: Allows scroll/taps to reach content below

                    // 2. The Enrichment Banner floating at the bottom
                    EnrichmentBanner(
                    ...
                    .contentShape(Rectangle())
                    .allowsHitTesting(true) // The banner itself remains interactive
...
// EnrichmentBanner now implemented in CSVImport/BackgroundImportBanner.swift
